...
import com.xter.support.entity.LogEvent;
...
//		}
	...
	public static void l(int prior, Level level, String msg) {
		if (DEBUG)
			RxBus.getDefault().post(new LogEvent(SysUtil.getNow(), level, getMethodPath(prior, prior) + msg));
		else
			RxBus.getDefault().post(new LogEvent(SysUtil.getNow(), level, msg));
	}
}
